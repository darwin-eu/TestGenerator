...
- [`checkRemoteFileAvailable()`](checkRemoteFileAvailable.md) : Check if
  a given remote file is available for download
- [`checkTablesColumns()`](checkTablesColumns.md) : Check if the given
  tables and columns are valid and return the loaded data.
- [`cleanupTestCdm()`](cleanupTestCdm.md) : Clean up a test CDM on a
  remote database
...
- [`readPatients()`](readPatients.md) : Converts a sample of patients
  into Unit Testing Definition JSON file.
- [`readPatients.csv()`](readPatients.csv.md) : Converts a sample of
